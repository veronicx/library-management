       ...
       <vue-good-table
            ...
            <span v-if="user.data.isAdmin && props.column.field === 'actions'">
                <mdb-dropdown end tag="li" class="nav-item">
                    ...
                    <mdb-dropdown-menu>
                        <mdb-dropdown-item @click.native="showModal=true;selectedUser=props.row;"><mdb-icon icon="trash" class="mr-3" />Delete</mdb-dropdown-item>
                        <mdb-dropdown-item :to="{name: 'editUser', params: {id: props.row._id}}"><mdb-icon icon="pen" class="mr-3" />Edit</mdb-dropdown-item>
                    </mdb-dropdown-menu>
                </mdb-dropdown>
        ...
        <div>
            ...
            <mdb-modal-body>Are you sure you want to delete selected user?</mdb-modal-body>
            <mdb-modal-footer>
                <mdb-btn color="primary" @click.native="showModal = false">Close</mdb-btn>
                <mdb-btn color="danger" @click.native="deleteUser(selectedUser)">Delete</mdb-btn>
            </mdb-modal-footer>
            </mdb-modal>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            async deleteUser(user) {
                console.log(user)
                await axios.delete(`http://localhost:8000/api/users/deleteUser/${user._id}`)
